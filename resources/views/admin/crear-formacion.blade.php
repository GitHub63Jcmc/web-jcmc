            ...
            <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.formacion.guardar') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="centro_formacion" name="centro_formacion" type="text" class="mt-1 block w-full" required />
                    </div>

                    <div>
                        <x-input-label for="titulo_curso" :value="__('Título del Curso')" />
                        <x-text-input id="titulo_curso" name="titulo_curso" type="text" class="mt-1 block w-full" required />
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input id="horas_formativas" name="horas_formativas" type="number" class="mt-1 block w-full" />
                    </div>

                    <div>
                        <x-input-label for="horas_pnl" :value="__('Horas PNL')" />
                        <x-text-input id="horas_pnl" name="horas_pnl" type="number" class="mt-1 block w-full" />
                    </div>

                    <div>
                        <x-input-label for="familia_profesional" :value="__('Familia Profesional')" />
                        <x-text-input id="familia_profesional" name="familia_profesional" type="text" class="mt-1 block w-full" />
                    </div>

                    <div>
                        <x-input-label for="codigo_curso" :value="__('Código del Curso')" />
                        <x-text-input id="codigo_curso" name="codigo_curso" type="text" class="mt-1 block w-full" />
                    </div>

                    <div class="md:col-span-2">
                        <x-input-label for="financiado_por" :value="__('Financiado por')" />
                        <x-text-input id="financiado_por" name="financiado_por" type="text" class="mt-1 block w-full" />
                    </div>
